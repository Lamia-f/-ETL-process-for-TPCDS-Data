-- The CUSTOMER_SNAPSHOT table is designed to store snapshot data for customer records.
-- It includes demographic details as well as transaction and contact information.CREATE OR REPLACE TABLE TPCDS.INTERMEDIATE.CUSTOMER_SNAPSHOT (
...
-- The CUSTOMER_DIM table holds the current, active dimension records for customer data.
-- It integrates various aspects of customer information for analytical purposes.create or replace TABLE TPCDS.ANALYTICS.CUSTOMER_DIM (
C_SALUTATION VARCHAR(16777216),
C_PREFERRED_CUST_FLAG VARCHAR(16777216),
C_FIRST_SALES_DATE_SK NUMBER(38, 0),
C_CUSTOMER_SK NUMBER(38, 0),
...
-- The WEEKLY_SALES_INVENTORY table aggregates sales and inventory data on a weekly basis.
-- It helps in analyzing sales trends, inventory levels, and stock shortages.
create
or replace TABLE TPCDS.ANALYTICS.WEEKLY_SALES_INVENTORY (
...
-- The DAILY_AGGREGATED_SALES table captures daily sales metrics per item and warehouse.
-- It's essential for understanding daily sales performance and for incremental updates.
create
or replace TABLE TPCDS.INTERMEDIATE.DAILY_AGGREGATED_SALES (
    WAREHOUSE_SK NUMBER(38, 0),
    ITEM_SK NUMBER(38, 0),
...
-- Cloning the ITEM dimension table for analytical use with consistent structure as the raw data.
create
or replace TABLE TPCDS.ANALYTICS.ITEM_DIM CLONE TPCDS.RAW.ITEM;
-- Cloning the WAREHOUSE dimension table for analytics; ensures structure matches the source table.
create
or replace TABLE TPCDS.ANALYTICS.WAREHOUSE_DIM CLONE TPCDS.RAW.WAREHOUSE;
-- Cloning the DATE dimension table to maintain a consistent calendar reference for reporting and analysis.
create
or replace TABLE TPCDS.ANALYTICS.DATE_DIM CLONE TPCDS.RAW.DATE_DIM;